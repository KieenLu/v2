...
def saveAndExit(event=0):
    ...
    ID = (textID.get())
    if (len(sys.argv) < 2):
        filepath = "image_for_attendance_system/"+name + "_" + ID + ".png"
    else:
        filepath = sys.argv[1]
    ...
    enroll_encoding.append(face_recognition.face_encodings(face_recognition.load_image_file(filepath))[0])
    with open('data/' +name+ "_"+ID+ '.txt', 'wb') as fp:
        # face_data = [enroll_encoding, known_face_metadata]
